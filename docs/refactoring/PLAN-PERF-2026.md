...
| 9 | Zoneless | ✅ **flip livré** (gate « ≥ 1 cycle de release en prod » **levé sur décision du 2026-08-14**) — lint + 892 specs + build mono-fichier ; **main.js 3,46 → 3,42 Mo (gzip 954 → 942,4 Ko)**. `zoneless: true` dans tous les environnements, zone.js sorti du bundle via le shim `src/zone-polyfill.ts` (no-op importé en tête de main.ts), politique `'zone'` supprimée de PluginBase (`ListenerZonePolicy = 'schedule' \| 'none'`, défaut `'schedule'` = handler + markForCheck coalescé ; les 2 listeners timeline ex-'zone' basculés). Karma garde zone.js + zone.js/testing. **Secours `zoneful`** : `ng build -c zoneful` (fileReplacements → `zone-polyfill.zoneful.ts` + `environment.zoneful.ts`, options prod) reproduit le bundle zoné — l'option `polyfills` d'angular.json est proscrite (elle émettrait un `polyfills.js` séparé, violation du contrat mono-fichier). Config `zoneless` d'angular.json supprimée (devenue le défaut) | 2026-08-14 | |
| 10 | Budgets finaux, CHANGELOG, docs | ✅ (budgets initial 3,7/4 Mo + anyComponentStyle 100/200 Ko ; CHANGELOG 2.1.26-develop ; PLAYER_EXPERT_INTEGRATION.md corrigé — le concat n'existe plus ; environment.prod.ts corrigé production: true) | 2026-08-10 | |
| 8bis | **Correctif post-smoke : rendu différé de la transcription réellement différé** | ✅ (lint + **892 specs** dont 5 nouvelles + build mono-fichier ; 3,46 Mo / 954,0 Ko gzip) — le surlignage des entités nommées passait par `querySelectorAll` et forçait l'hydratation de **tous** les blocs `@defer` dès qu'un segment portait une annotation (cas dominant : 127/151 segments sur un flux TV réel). Il est désormais marqué sur les données (`TranscriptionLocalisation.isNamedEntity` via `markNamedEntities`, appelé par `parseTranscription`) et rendu par `[class.named-entity]`. **Mesuré sur le flux TV réel** (151 segments, 127 annotés, 10 405 mots) : **10 405 → 413 mots rendus au chargement (−96 %), 12 955 → 3 109 nœuds DOM (−76 %)**, 146 placeholders restaurés, `forceRenderAll` déclenché par la seule recherche ; parité vérifiée (597 surlignages = compte exact des données après hydratation globale). Contrepartie : la 1re recherche passe de ~190 ms à ~3,5 s (coût déplacé du chargement vers la demande). Suppression au passage de 4 helpers DOM et du `Utils.waitFor` de `ngAfterViewInit` | 2026-08-12 | |
| — | **Correctif post-smoke : filigrane audio et contrat mono-fichier** | ✅ (lint + 892 specs + build ; `dist/amalia/media/` absent, garde du build étendue) — le SCSS référençait `newAudioBackGround.png` en `url()` relative, externalisée par esbuild dans `dist/amalia/media/` → 404 en intégration (le bundle est servi depuis `/assets/`). Filigrane désormais vectoriel **inline** ; `player-expert` ne fournit plus ni `poster` ni globs dédiés. Vérifié sur le flux radio réel : plus aucune 404 | 2026-08-12 | |
| — | **Correctif post-smoke : export Excel des segments (interop json-as-xlsx)** | ✅ (lint + 892 specs + build mono-fichier ; 3,42 Mo stable, +12 octets) — `import * as xlsx from 'json-as-xlsx'` appelé comme fonction : webpack passait le `module.exports` CJS appelable, le `__toESM` d'esbuild (phase 2) fabrique un namespace non appelable → `TypeError` au clic (invisible aux specs, `callXlsx` toujours espionné). Correctif : import par défaut. Vérifié en réel dans `player-expert` (classeur 51,8 Ko, console propre) — détail dans [SMOKE-CHECKLIST.md](SMOKE-CHECKLIST.md), passe du 2026-08-17 | 2026-08-17 | |
| — | **Smoke visuel avant merge** | ✅ **complet** — 2 passes du 2026-08-12 + passes du 2026-08-17 dans [SMOKE-CHECKLIST.md](SMOKE-CHECKLIST.md) : (1) hors réseau, sur harnais isolé — control-bar experte, transcription, timeline/interactjs, toast, 0 fuite de listeners ; 6 anomalies rejouées sur le bundle pré-chantier 2.1.24 ⇒ toutes pré-existantes. (2) **sur réseau INA dans `player-expert`** (bundle identique au bit près au build de branche) — HLS/hls.js, `backwardsSrc`, vignettes de survol, plein écran réel, storyboard réel, **waveform wavesurfer**, **mode photo/cropperjs** tous ✅. (3) **annotation/segments sur flux réel ET stock réel** (2026-08-17) — CRUD, chips/autocomplete, dialog de confirmation, imagette, exports JSON + Excel tous ✅ sur les deux natures (régression Excel corrigée au passage ; le 500 backend du cas stock est corrigé, `POST /segments/stock` → 200). (4) **Mesure M2 avant/après** (2026-08-17, traces CDP sur harnais offline, `scripts/perf-m2-analyse.mjs`) — scripting stable (~28 ms/s en lecture) mais **occupation du thread principal ÷ 2,5** (32 % → 13 % en lecture, ÷ 3 par mousemove) ; seek en rafale plus cher (hydratation `@defer` par seek, contrepartie assumée de la phase 8). (5) **Mesure M1** (2026-08-17, build dev + `ng.ɵsetProfiler`) — **cible phase 9 atteinte** : 0 tick à l'arrêt, **38 ticks pour 38 `timeupdate`** en 10 s de lecture (1 tick coalescé/timeupdate), seuls time-bar + control-bar re-rendus (player/transcriptions/storyboard : 0), 0 tick sous 191 mousemove à l'arrêt | 2026-08-17 | |
...
**Gates systématiques** : `ng lint` + `npm test` (892 specs vertes en fin de chantier) + `npm run build:component` + `node scripts/size-report.mjs` + [SMOKE-CHECKLIST.md](SMOKE-CHECKLIST.md).
...
> **Livrée le 2026-08-14.** Le gate « phases 1-8 en prod ≥ 1 cycle de release » a été **levé sur décision**
> (les autres gates étaient soldés). Écarts d'implémentation vs le plan ci-dessus :
> - le retrait de zone.js passe par un shim `src/zone-polyfill.ts` (no-op, importé en tête de main.ts)
>   remplacé par `zone-polyfill.zoneful.ts` (`import 'zone.js'`) dans la configuration de secours `zoneful` —
>   l'option `polyfills` d'angular.json était inutilisable car elle émet un `polyfills.js` séparé,
>   en violation du contrat mono-fichier ;
> - `bootstrap.ts` conserve le ternaire `environment.zoneless ? zoneless : zone+coalescing` : c'est lui
>   que la config `zoneful` (fileReplacements → `environment.zoneful.ts`) exploite pour le rollback ;
> - `ListenerZonePolicy` réduit à `'schedule' | 'none'` (défaut `'schedule'`) : sans zone.js, l'ancienne
>   politique `'zone'` dégénérait déjà en `'schedule'` (NgZone noop) — comportement identique, y compris
>   pour les 2 listeners timeline METADATA_LOADED/USER_SEGMENT_CHANGED qui étaient restés en `'zone'` ;
> - les TestBed restent zonés (zone.js + zone.js/testing conservés côté Karma), `bootstrap.spec.ts`
>   boot désormais réellement zoneless via environment.ts.
