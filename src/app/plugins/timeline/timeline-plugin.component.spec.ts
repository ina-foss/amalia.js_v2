...
describe("TimelinePluginComponent", () => {
    ...
    it("should preserve the timeline filter when metadata is rebuilt with new ids", () => {
        ...
        component.selectedNodes.set([component.nodes[1], component.nodes[1].children[0]]);
        component.allNodesChecked = false;
        ...
        expect(component.selectedNodes().map((node) => node.key)).toEqual([newScheduleId, DataType.DAY_SCHEDULE]);
        expect(component.allNodesChecked).toBeFalse();
                    ...
                    localisation: [
                        {
                            label: "Parent",
                            sublocalisations: {
                                localisation: [{ label: undefined }, { label: "Child with label" }],
                            },
                        },
                    ],
                },
...
describe("TimelinePluginComponent 3 ", () => {
    ...
    it("should export tv days", () => {
        ...
        expect(component.mediaPlayerElement.eventEmitter.emit).toHaveBeenCalledWith(
            PlayerEventType.TIMELINE_EXPORT_TV_DAYS,
        );
    });
});
