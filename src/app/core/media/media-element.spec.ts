...
describe('Test Media element', () => {
    ...
    it('clampe setCurrentTime dans [0, duration] quand la durée est connue', () => {
        const video = document.createElement('video');
        Object.defineProperty(video, 'duration', {configurable: true, value: 20});
        const mediaElement = new MediaElement(video, new EventEmitter());

        mediaElement.setCurrentTime(150);
    ...
    it('ne clampe pas vers le haut quand la durée est inconnue (NaN)', () => {
        const video = document.createElement('video'); // sans média : duration = NaN
        const mediaElement = new MediaElement(video, new EventEmitter());

        mediaElement.setCurrentTime(42);
        expect(video.currentTime).toBe(42);
    });

    it('les sauts par frames s\'arrêtent sur la dernière frame, jamais à duration', () => {
        const video = document.createElement('video');
        Object.defineProperty(video, 'duration', {configurable: true, value: 20});
        const mediaElement = new MediaElement(video, new EventEmitter());
        mediaElement.framerate = 10; // dernière frame à 19.9

        video.currentTime = 18;
        mediaElement.moveNextFrame(100); // +10 s demandés : clamp sur la dernière frame
        expect(video.currentTime).toBeCloseTo(19.9, 5);

        mediaElement.moveNextFrame(10); // déjà au bout : reste sur la dernière frame
        expect(video.currentTime).toBeCloseTo(19.9, 5);

        // seekToEnd conserve, lui, le droit d'aller à duration pile (état ended assumé)
        mediaElement.seekToEnd();
        expect(video.currentTime).toBe(20);
    });
