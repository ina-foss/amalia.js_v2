...
export class MediaElement {
    ...
    public setCurrentTime(value: number): void {
        ...
        if (this.mediaElement) {
            // Clamp haut seulement si la durée est connue et finie (métadonnées non
            // chargées → NaN, live → Infinity) — aligne le code sur le clamp natif.
            ...
            this.mediaElement.currentTime = Math.min(maxTime, Math.max(0, currentTime));
    ...
    /**
     * Borne un timecode de navigation par frames sur [0, duration - 1/framerate] :
     * les sauts (±5s/±10s, pas-à-pas) ne doivent jamais atterrir exactement à duration —
     * cela déclencherait l'état ended (arrêt de lecture) en pleine navigation rapide.
     * seekToEnd, lui, garde le droit d'aller à duration.
     */
    private clampToLastFrame(value: number): number {
        const duration = this.mediaElement?.duration;
        if (Number.isFinite(duration) && duration > 0) {
            return Math.min(value, Math.max(0, duration - 1 / this.framerate));
        }
        return value;
    }
    ...
    moveNextFrame(nbFrames = 1) {
        ...
        this.setCurrentTime(this.clampToLastFrame(Math.max(0, currentTime)));
    }
