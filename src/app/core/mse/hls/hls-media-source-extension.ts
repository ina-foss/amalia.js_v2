...
export class HLSMediaSourceExtension implements MediaSourceExtension {
    ...
    constructor(mediaElement: HTMLVideoElement, eventEmitter: EventEmitter, config: PlayerConfigData, logger: LoggerInterface) {
        ...
        Utils.addListener(this, this.eventEmitter, PlayerEventType.ACCURATE_SEEK_CHANGE, this.handleAccurateSeekChange);
    }
    ...
    private handleAudioChannelChange(event) {
        ...
        this.logger.debug(`[AUDIO_TRACK_DEBUG] handleAudioChannelChange appliqué sur cette instance HLS -> audioChannel=${event}`);
        this.hlsPlayer.config.fragLoadPolicy.default['audioChannel'] = event;
    }

    /**
     * Invoked on accurate-seek toggle — armed by the control bar at slide-mode mouseup only
     * (the drag's live seeks stay on normal fragments), consumed one-shot by
     * CustomFragmentLoader on the landing fragment.
     * @param enabled true while the next fragment must carry accurate_seek=1
     */
    private handleAccurateSeekChange(enabled: boolean) {
        this.logger.debug('handleAccurateSeekChange', enabled);
        this.hlsPlayer.config.fragLoadPolicy.default['accurateSeek'] = enabled;
    }
