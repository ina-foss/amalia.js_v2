import {waitForAsync, fakeAsync, getTestBed, TestBed, tick, flush} from '@angular/core/testing';
...
describe('Test Metadata manager', () => {
    ...
    beforeEach(waitForAsync(() => {
        ...
        const conf: PluginConfigData<TimeBarConfig> = {
            name: TimeBarPluginComponent.PLUGIN_NAME,
            data: {timeFormat: 'f', theme: 'outside'}
        };
    ...
    it('Test MetaDataManager loadDataSourceForPlugin TimeLines', fakeAsync(() => {
        configData = require('tests/assets/config-mpe-with-timeline-info.json');
        ...
        configurationManager.load(configData).then(() => {
            metadataManager.loadDataSourceForPlugin('timelines').then(
                    () => {
                        const metadataSegmentation = metadataManager.getMetadataByType(`${DataType.SEGMENTATION}-`);
                        expect(metadataSegmentation.length).toEqual(5);
                        expect(metadataSegmentation[0].type).toContain(DataType.SEGMENTATION);

                        const metadataRecoFaciale = metadataManager.getMetadataByType(`${DataType.FACES_RECOGNITION}-`);
                        expect(metadataRecoFaciale.length).toEqual(56);
                        expect(metadataRecoFaciale[0].type).toContain(DataType.FACES_RECOGNITION);
                    }
            ).catch(() => {
                fail('Error to call assert');
            });
    ...
    it('Test MetaDataManager loadDataSourceForPlugin Annotations', fakeAsync(() => {
        ...
        configurationManager.load(configData).then(() => {
            metadataManager.loadDataSourceForPlugin('annotations').then(
                    () => {
                        const metadata = metadataManager.getMetadata('annotations');
                        expect(metadata).not.toBeNull();
                        expect(metadata).not.toBeUndefined();
                    }
            ).catch((error) => {
                fail(`Error to call assert ${error}`);
            });
    ...
    it('Test MetaDataManager refreshDataSourceHeaders Annotation', () => {
        ...
        const newToken = 'eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICItWm1vNDlTcXlOdEJnREJUMU5oLXdTczhiNHpVLTloWE0wS3VlT0tKTVI0In0..SE45rgPoE0oxziUZxxF_PzOd63Wy3AhawwRgX0FC7fnauV2Rq5YoziwbwkxYpE-AJ1x-rNPLkEe1Sjdncq7MM87GqewmEa_WcYOCYwZzVKW1Npl0Pzbf7D0bNzXCCh80owsw9NjhT_LnL0uqUf7q7DFbKiRExS8IXKv2WF0RYC7dCOkJ3bSXjJ8I1qhcIOPdiuTIKRIccoHGopZGrRtf2iV8a6pg1MnxQvh3-G33Kjo0jC7D7dU1alQ_rLm1eQaHrM1zET6hEYXN3eMoR2Es_ZrgMqZjNB9YlI4HZ4QUM56sxvZ5zefKuTMKuzCDbEf_USBz7iH1w1BovUB6AIlkcg';

        expect(oldToken).not.toEqual(newToken);
        expect(configData.dataSources.length).toEqual(3);
        configData.dataSources.forEach(dataSource => {
            expect(dataSource.headers.length).not.toEqual(0);
            dataSource.headers.forEach((header) => {
                if (header.includes(MetadataManager.AUTHORIZATION_HEADER)) {
                    expect(header).toEqual(`${MetadataManager.AUTHORIZATION_HEADER} ${oldToken}`);
                }
            });
        });
        configurationManager.load(configData).then(() => {
            metadataManager.refreshDataSourceHeaders(newToken);
            expect(configData.dataSources.length).toEqual(3);
            configData.dataSources.forEach(dataSource => {
                expect(dataSource.headers.length).not.toEqual(0);
                dataSource.headers.forEach((header) => {
                    if (header.includes(MetadataManager.AUTHORIZATION_HEADER)) {
                        expect(header).toEqual(`${MetadataManager.AUTHORIZATION_HEADER} ${newToken}`);
                    }
                });
            });
        });
