...
import { quantizeThumbnailTc } from './utils/thumbnail-tc';
...
export class MediaPlayerElement {
    ...
    public getThumbnailUrl(tc: number, onHover?: boolean) {
        if (this.getConfiguration().thumbnail.enableThumbnail && this.getConfiguration().thumbnail?.baseUrl) {
            const thumbnailConfig = this.getConfiguration().thumbnail;
            const baseUrl = thumbnailConfig.baseUrl;
            const tcParam = thumbnailConfig.tcParam ? thumbnailConfig.tcParam : 'start';
            // onHover => petite vignette (survol + éclaireur rafale), sinon grande (overlay plein cadre)
            const widthParam = onHover ? thumbnailConfig.width : thumbnailConfig.previewWidth;
            const roundedTc = quantizeThumbnailTc(tc);
            const separator = baseUrl.search('\\?') === -1 ? '?' : '&';
            // Un hôte non migré qui soude déjà ?width=N dans baseUrl garde la priorité (pas de double width=)
            if (widthParam > 0 && !/[?&]width=/.test(baseUrl)) {
                return `${baseUrl}${separator}width=${widthParam}&${tcParam}=${roundedTc}`;
            }
            return `${baseUrl}${separator}${tcParam}=${roundedTc}`;
        }
        return undefined;
    }
