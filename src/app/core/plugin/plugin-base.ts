...
export abstract class PluginBase<T> implements OnInit {
    ...
    public metaDataLoaded(): boolean {
        ...
        if (metadataManager && handleMetadataIds && Utils.isArrayLike<string>(handleMetadataIds)) {
            handleMetadataIds.forEach((metadataId) => {
                ...
                if (!metadataManager.getMetadata(metadataId)) {
                    result = false;
                }
            });
        } else {
    ...
    ngOnInit(): void {
        ...
        this.subscriptionToEventsEmitters.push(Utils.waitFor(this.metaDataLoaded.bind(this),
                this.handleMetadataLoaded.bind(this),
                this.logWaitForMetaDataToBeLoadedComplete.bind(this),
                this.intervalStep,
                this.timeout,
                this.setDataLoading.bind(this)));

        this.subscriptionToEventsEmitters.push(Utils.waitFor(this.mediaPlayerElementReady.bind(this),
                this.setTcOffset.bind(this),
