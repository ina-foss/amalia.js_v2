...
- **Smoke visuel avant merge** : 2 passes déroulées le 2026-08-12, plus la passe **annotation/segments** du 2026-08-17 (CRUD, chips/autocomplete, dialog, imagette, exports JSON + Excel — sur flux réel **et** stock réel dans `player-expert`, le 500 backend du cas stock étant corrigé) ; résultats et méthode dans `docs/refactoring/SMOKE-CHECKLIST.md`. Cette passe a mis au jour et corrigé une **régression de l'export Excel** (interop CJS `json-as-xlsx` cassée par le passage webpack → esbuild : `import * as` d'un CJS appelé comme fonction — motif à auditer statiquement pour tout nouveau paquet CJS). Les mesures **M1 et M2** ont été relevées le 2026-08-17, ce qui **solde le smoke avant merge** : M2 (traces CDP avant/après, dépouillement par `scripts/perf-m2-analyse.mjs`) — scripting stable mais **occupation du thread principal ÷ 2,5** en lecture ; M1 (build dev + `ng.ɵsetProfiler`) — **cible phase 9 atteinte** : 0 tick à l'arrêt, 1 tick coalescé par `timeupdate` en lecture, seuls time-bar et control-bar re-rendus, 0 tick sous mousemove.
