...
- **Smoke visuel avant merge** : 2 passes déroulées le 2026-08-12, plus la passe **annotation/segments** du 2026-08-17 (CRUD, chips/autocomplete, dialog, imagette, exports JSON + Excel — sur flux réel **et** stock réel dans `player-expert`, le 500 backend du cas stock étant corrigé) ; résultats et méthode dans `docs/refactoring/SMOKE-CHECKLIST.md`. Cette passe a mis au jour et corrigé une **régression de l'export Excel** (interop CJS `json-as-xlsx` cassée par le passage webpack → esbuild : `import * as` d'un CJS appelé comme fonction — motif à auditer statiquement pour tout nouveau paquet CJS). La mesure **M2** a été relevée le 2026-08-17 (traces CDP avant/après sur le harnais offline, dépouillement par `scripts/perf-m2-analyse.mjs`) : scripting stable, mais **occupation du thread principal ÷ 2,5** en lecture. Reste à couvrir : la mesure M1 (ticks CD — exige un build dev : `npm start` + Angular DevTools ou `ng.ɵsetProfiler`).
