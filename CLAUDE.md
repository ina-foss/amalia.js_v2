...
- **Smoke visuel avant merge** : 2 passes déroulées le 2026-08-12, plus la passe **annotation/segments** du 2026-08-17 (CRUD, chips/autocomplete, dialog, imagette, exports JSON + Excel — sur flux réel dans `player-expert`) ; résultats et méthode dans `docs/refactoring/SMOKE-CHECKLIST.md`. Cette passe a mis au jour et corrigé une **régression de l'export Excel** (interop CJS `json-as-xlsx` cassée par le passage webpack → esbuild : `import * as` d'un CJS appelé comme fonction — motif à auditer statiquement pour tout nouveau paquet CJS). Restent à couvrir : les mesures Angular DevTools M1/M2 et le cas stock (`POST /api/dossier/segments/stock`, 500 backend dev).
